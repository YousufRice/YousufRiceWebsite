...
if (typeof window === 'undefined' && process.env.APPWRITE_API_KEY) {
  try {
    ...
    const { Client: ServerClient, Databases: ServerDatabases, Storage: ServerStorage } = require('node-appwrite');
...
export const ORDERS_TABLE_ID = process.env.NEXT_PUBLIC_APPWRITE_ORDERS_TABLE_ID!;
export const CUSTOMERS_TABLE_ID = process.env.NEXT_PUBLIC_APPWRITE_CUSTOMERS_TABLE_ID!;
export const ADDRESSES_TABLE_ID = process.env.NEXT_PUBLIC_APPWRITE_ADDRESSES_TABLE_ID!;
export const PRODUCT_IMAGES_TABLE_ID = process.env.NEXT_PUBLIC_APPWRITE_PRODUCT_IMAGES_TABLE_ID!;
