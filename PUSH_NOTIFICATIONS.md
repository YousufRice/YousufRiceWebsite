...
json#### POST `/api/push/unsubscribe`
...
json#### GET `/api/push/status?endpoint=...`
...
json### Admin Endpoints (Protected by `PUSH_API_SECRET`)
...
language### 2. Generate Push API Secret
...
language### 3. Run Setup Script

```bash
pnpm dotenv tsx scripts/setup-push-tables.ts
```

bash### 4. Add Service Worker
