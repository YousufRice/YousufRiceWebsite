...
export default function CheckoutPage() {
  ...
  const [payFastForm, setPayFastForm] = useState<{ url: string; data: any } | null>(null);
  ...
  useEffect(() => {
    if (payFastForm && payFastFormRef.current) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await databases.createDocument(DATABASE_ID, ORDERS_TABLE_ID, orderId, {
        ...
        status: "pending",
        online_payment_status: paymentMethod === "online" ? "pending" : "",
      });
      ...
      if (paymentMethod === "online") {
        try {
          const response = await fetch("/api/payfast/token", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              basket_id: orderId,
              txnamt: finalTotalPrice,
          ...
          // Set form data to trigger auto-submit
          setPayFastForm({ url: data.postUrl, data: data.formData });

          // Clear cart but don't redirect yet (form submit will redirect)
          clearCart();
          isOrderPlacedRef.current = true;
          return; // Stop execution here, let the form submit
        } catch (paymentError: any) {
          console.error("PayFast Error:", paymentError);
          toast.error(`Payment initiation failed: ${paymentError.message}`);
          // Revert order or show error? For now just show error.
          // Ideally we might want to delete the pending order or let user retry.
          setLoading(false);
          return;
        }
      }
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-gray-50 to-white">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div
                        ...
                        className={`cursor-pointer border-2 rounded-xl p-4 flex flex-col items-center justify-center gap-2 transition-all ${paymentMethod === "cod"
                          ? "border-[#27247b] bg-[#27247b]/5"
                          : "border-gray-200 hover:border-[#27247b]/50"
                          }`}
                      >
                        <span className="text-2xl">💵</span>
                        <span className={`font-bold ${paymentMethod === "cod" ? "text-[#27247b]" : "text-gray-600"}`}>
                          Cash on Delivery
                        </span>
                      </div>
                      ...
                      <div
                        ...
                        className={`cursor-pointer border-2 rounded-xl p-4 flex flex-col items-center justify-center gap-2 transition-all ${paymentMethod === "online"
                          ? "border-[#27247b] bg-[#27247b]/5"
                          : "border-gray-200 hover:border-[#27247b]/50"
                          }`}
                      >
                        <span className="text-2xl">💳</span>
                        <span className={`font-bold ${paymentMethod === "online" ? "text-[#27247b]" : "text-gray-600"}`}>
                          Pay Online
                        </span>
                      </div>
                    </div>
              ...
              <CardContent className="p-6">
                ...
                <div className="border-t-2 border-gray-200 pt-4">
                  ...
                  <Button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-[#27247b] hover:bg-[#27247b]/90 text-white font-bold py-6 text-lg rounded-xl shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {loading ? (
      ...
      {/* Hidden Form for PayFast Redirection */}
      {payFastForm && (
        <form
          ref={payFastFormRef}
          action={payFastForm.url}
          method="POST"
          className="hidden"
        >
          {Object.entries(payFastForm.data).map(([key, value]) => (
