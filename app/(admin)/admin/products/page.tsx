...
import { Plus, Edit, Trash2, Image as ImageIcon, Images, Search, Filter, Grid3x3, List, Download, Upload, CheckCircle2, XCircle, TrendingUp, Package } from 'lucide-react';
...
export default function AdminProductsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const productData = {
        ...
        tier_2_4kg_price: formData.has_tier_pricing && formData.tier_2_4kg_price ? roundPrice(formData.tier_2_4kg_price) : null,
        tier_5_9kg_price: formData.has_tier_pricing && formData.tier_5_9kg_price ? roundPrice(formData.tier_5_9kg_price) : null,
        tier_10kg_up_price: formData.has_tier_pricing && formData.tier_10kg_up_price ? roundPrice(formData.tier_10kg_up_price) : null,
        available: formData.available
      };

      if (editingId) {
        await databases.updateDocument(DATABASE_ID, PRODUCTS_TABLE_ID, editingId, productData);
        toast.success('Product updated successfully!');
      } else {
        const productId = ID.unique();
        await databases.createDocument(DATABASE_ID, PRODUCTS_TABLE_ID, productId, productData);
        toast.success('Product created! Now add images by clicking the 📸 icon.');
      }
  ...
  const handleBulkToggleAvailability = async (available: boolean) => {
    ...
    try {
      await Promise.all(
        selectedProducts.map(id => 
          databases.updateDocument(DATABASE_ID, PRODUCTS_TABLE_ID, id, { available })
        )
      );
  ...
  const stats = {
    ...
    avgPrice: products.length > 0 ? products.reduce((sum, p) => sum + p.base_price_per_kg, 0) / products.length : 0
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, has_tier_pricing: e.target.checked })}
                  className="w-4 h-4 text-green-600 rounded focus:ring-green-500"
                />
                <label htmlFor="has_tier_pricing" className="text-sm font-medium text-gray-700">
                  Enable Tier Pricing
                </label>
              </div>
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>📸 Note:</strong> After creating the product, click the <Images className="w-4 h-4 inline text-blue-600" /> icon to manage product images.
                </p>
              </div>
              ...
              <CardContent className="p-4">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex space-x-1">
                    ...
                    <ReadOnlyGuard>
                      <Button size="sm" variant="ghost" onClick={() => handleEdit(product)} title="Edit Product">
                        <Edit className="w-4 h-4" />
                      </Button>
                    </ReadOnlyGuard>
                    <ReadOnlyGuard>
                      <Button size="sm" variant="ghost" onClick={() => handleDelete(product.$id)} title="Delete Product">
                ...
                <div className="space-y-1 mb-3">
                  <p className="text-sm">
                    <span className="font-medium">Base Price:</span> {formatCurrency(product.base_price_per_kg)}/kg
                  </p>
                  {product.has_tier_pricing && (
                    <>
                      {product.tier_2_4kg_price && (
                        <p className="text-xs text-gray-600">2-4kg: {formatCurrency(product.tier_2_4kg_price)}/kg</p>
                      )}
                      {product.tier_5_9kg_price && (
                        <p className="text-xs text-gray-600">5-9kg: {formatCurrency(product.tier_5_9kg_price)}/kg</p>
                      )}
                      {product.tier_10kg_up_price && (
                        <p className="text-xs text-gray-600">10+kg: {formatCurrency(product.tier_10kg_up_price)}/kg</p>
                      )}
                    </>
                  )}
                </div>
              ...
              <TableBody>
                {filteredProducts.map((product) => (
                  <TableRow key={product.$id}>
                    ...
                    <TableCell className="font-medium">{formatCurrency(product.base_price_per_kg)}/kg</TableCell>
                    <TableCell>
                      {product.has_tier_pricing ? (
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <ReadOnlyGuard>
                          <Button size="sm" variant="ghost" onClick={() => handleDelete(product.$id)} title="Delete">
                            <Trash2 className="w-4 h-4 text-red-500" />
                          </Button>
                        </ReadOnlyGuard>
                      </div>
